...
int ljForce(SimFlat* s)
{
	...
	#endif
	#pragma acc kernels present(s->atoms->f[0:fSize][0:3], s->atoms->U[0:fSize])
	...
	#pragma pointerchain region begin
	#endif
	#pragma acc parallel loop reduction(+:ePot) present(s->boxes->nAtoms[0:s->boxes->nLocalBoxes], \
		...
		for (int jTmp=0; jTmp<nNbrBoxes; jTmp++)
		{
			...
			for (int iOff=MAXATOMS*iBox; iOff<(iBox*MAXATOMS+nIBox); iOff++)
			{
				...
				for (int jOff=jBox*MAXATOMS; jOff<(jBox*MAXATOMS+nJBox); jOff++)
				{
					real3 dr;
					real_t r2 = 0.0;
					dr[0] = s->atoms->r[iOff][0]-s->atoms->r[jOff][0];
					r2+=dr[0]*dr[0];
					dr[1] = s->atoms->r[iOff][1]-s->atoms->r[jOff][1];
					r2+=dr[1]*dr[1];
					dr[2] = s->atoms->r[iOff][2]-s->atoms->r[jOff][2];
					r2+=dr[2]*dr[2];
					...
					if ( r2 <= rCut2 && r2 > 0.0)
					{
						...
						s->atoms->f[iOff][0] -= dr[0]*fr;
						s->atoms->f[iOff][1] -= dr[1]*fr;
						s->atoms->f[iOff][2] -= dr[2]*fr;
					}
				} // loop over atoms in jBox
			} // loop over atoms in iBox
