...
int loadAtomsBuffer(void* vparms, void* data, int face, char* charBuf)
{
   ...
   for (int iCell=0; iCell<nCells; ++iCell)
   {
      ...
      for (int ii=iOff; ii<iOff+s->boxes->nAtoms[iBox]; ++ii)
      {
         ...
         buf[nBuf].rx = s->atoms->rx[ii] + shift[0];
         buf[nBuf].ry = s->atoms->ry[ii] + shift[1];
         buf[nBuf].rz = s->atoms->rz[ii] + shift[2];
         buf[nBuf].px = s->atoms->px[ii];
         buf[nBuf].py = s->atoms->py[ii];
         buf[nBuf].pz = s->atoms->pz[ii];
...
void sortAtomsInCell(Atoms* atoms, LinkCell* boxes, int iBox)
{
   ...
   for (int ii=begin, iTmp=0; ii<end; ++ii, ++iTmp)
   {
	  ...
	  tmp[iTmp].rx =   atoms->rx[ii];
      tmp[iTmp].ry =   atoms->ry[ii];
      tmp[iTmp].rz =   atoms->rz[ii];
      tmp[iTmp].px =   atoms->px[ii];
      tmp[iTmp].py =   atoms->py[ii];
      tmp[iTmp].pz =   atoms->pz[ii];
   }
   ...
   for (int ii=begin, iTmp=0; ii<end; ++ii, ++iTmp)
   {
      ...
      atoms->rx[ii]  = tmp[iTmp].rx;
      atoms->ry[ii]  = tmp[iTmp].ry;
      atoms->rz[ii]  = tmp[iTmp].rz;
      atoms->px[ii]  = tmp[iTmp].px;
      atoms->py[ii]  = tmp[iTmp].py;
      atoms->pz[ii]  = tmp[iTmp].pz;
   }
