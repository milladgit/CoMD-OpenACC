...
static void sumAtoms(SimFlat* s, int on_gpu);
...
static void checkCoMDGangVectorSize(SimFlat *s);
...
SimFlat* initSimulation(Command cmd)
{
   ...
   redistributeAtomsInInit(sim);
   stopTimer(redistributeTimer);

   checkCoMDGangVectorSize(sim);
...
void sumAtoms(SimFlat* s, int on_gpu)
{
   // sum atoms across all processers
   s->atoms->nLocal = 0;

   #ifdef __pointerchain
   #pragma pointerchain declare(s->atoms->nLocal{int}, s->boxes->nLocalBoxes{int}, s->boxes->nAtoms{int*})
   #endif

   #ifdef __pointerchain
   #pragma pointerchain region begin
   #endif
   #pragma acc parallel loop gang vector reduction(+:s->atoms->nLocal) \
   	   	   	   present(s->boxes->nAtoms[0:s->boxes->nLocalBoxes]) if(on_gpu)
...
static void checkCoMDGangVectorSize(SimFlat *s) {
	s->gangCountF = s->gangCountAP = s->gangCountAV = -1;
	s->vectorCountF = s->vectorCountAP = s->vectorCountAV = -1;
	if(s->gangCountF == -1 || s->vectorCountF == -1) {
		char *str, *e;
		char temp[128];
		str = getenv("COMD_GANG_COUNT");
		if(str == NULL) {
			fprintf(stderr, "Please provide gang count in COMD_GANG_COUNT environment variable.\n");
			exit(0);
		}
		e = strchr(str, ',');
		if(e == NULL) {
			s->gangCountF = s->gangCountAP = s->gangCountAV = atoi(str);
		} else {
			int len;
			len = (e - str + 1);
			memcpy(&temp[0], str, len);
			temp[len] = NULL;
			s->gangCountF = atoi(&temp[0]);
			str += len;

			e = strchr(str, ',');
			if(e == NULL) { fprintf(stderr, "Please provide correct gang values.\n"); exit(0); }
			len = (e - str + 1);
			memcpy(&temp[0], str, len);
			temp[len] = NULL;
			s->gangCountAP = atoi(&temp[0]);
			str += len;

			s->gangCountAV = atoi(str);
		}
		if(s->gangCountF == 0) {
			fprintf(stderr, "Please provide gang count in COMD_GANG_COUNT environment variable.\n");
			exit(0);
		...
		str = getenv("COMD_VECTOR_COUNT");
		if(str == NULL) {
			fprintf(stderr, "Please provide vector count in COMD_VECTOR_COUNT environment variable.\n");
			exit(0);
		}
		e = strchr(str, ',');
		if(e == NULL) {
			s->vectorCountF = s->vectorCountAP = s->vectorCountAV = atoi(str);
		} else {
			int len;
			len = (e - str + 1);
			memcpy(&temp[0], str, len);
			temp[len] = NULL;
			s->vectorCountF = atoi(&temp[0]);
			str += len;

			e = strchr(str, ',');
			if(e == NULL) { fprintf(stderr, "Please provide correct vector values.\n"); exit(0); }
			len = (e - str + 1);
			memcpy(&temp[0], str, len);
			temp[len] = NULL;
			s->vectorCountAP = atoi(&temp[0]);
			str += len;

			s->vectorCountAV = atoi(str);
		}
		if(s->vectorCountF == 0) {
			fprintf(stderr, "Please provide gang count in COMD_VECTOR_COUNT environment variable.\n");
			exit(0);
		}
	}
}
