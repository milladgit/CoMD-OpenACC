...
void updateLinkCells(LinkCell* boxes, Atoms* atoms)
{
   ...
   for (int iBox=0; iBox<boxes->nLocalBoxes; ++iBox)
   {
      ...
      {
    	 real_t rr[3];
		 rr[0] = atoms->rx[iOff+ii];
		 rr[1] = atoms->ry[iOff+ii];
		 rr[2] = atoms->rz[iOff+ii];
...
//   memcpy(atoms->U+jOff,  atoms->U+iOff,  sizeof(real_t));
   atoms->rx[jOff] = atoms->rx[iOff];
   atoms->ry[jOff] = atoms->ry[iOff];
   atoms->rz[jOff] = atoms->rz[iOff];
   atoms->px[jOff] = atoms->px[iOff];
   atoms->py[jOff] = atoms->py[iOff];
   atoms->pz[jOff] = atoms->pz[iOff];
   atoms->fx[jOff] = atoms->fx[iOff];
   atoms->fy[jOff] = atoms->fy[iOff];
   atoms->fz[jOff] = atoms->fz[iOff];
