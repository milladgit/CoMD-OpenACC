...
Atoms* initAtoms(LinkCell* boxes)
{
   ...
   atoms->rx = 		 (real_t*) comdMalloc(maxTotalAtoms*sizeof(real_t));
   atoms->ry = 		 (real_t*) comdMalloc(maxTotalAtoms*sizeof(real_t));
   atoms->rz = 		 (real_t*) comdMalloc(maxTotalAtoms*sizeof(real_t));
   atoms->px = 		 (real_t*) comdMalloc(maxTotalAtoms*sizeof(real_t));
   atoms->py = 		 (real_t*) comdMalloc(maxTotalAtoms*sizeof(real_t));
   atoms->pz = 		 (real_t*) comdMalloc(maxTotalAtoms*sizeof(real_t));
   atoms->fx = 		 (real_t*) comdMalloc(maxTotalAtoms*sizeof(real_t));
   atoms->fy = 		 (real_t*) comdMalloc(maxTotalAtoms*sizeof(real_t));
   atoms->fz = 		 (real_t*) comdMalloc(maxTotalAtoms*sizeof(real_t));
   ...
   for (int iOff = 0; iOff < maxTotalAtoms; iOff++)
   {
      ...
      atoms->rx[iOff] = atoms->ry[iOff] = atoms->rz[iOff] = 0.0;
      atoms->px[iOff] = atoms->py[iOff] = atoms->pz[iOff] = 0.0;
      atoms->fx[iOff] = atoms->fy[iOff] = atoms->fz[iOff] = 0.0;
      atoms->U[iOff] = 0.;
   }
...
void destroyAtoms(Atoms *atoms)
{
   ...
   freeMe(atoms,rx);freeMe(atoms,ry);freeMe(atoms,rz);
   freeMe(atoms,px);freeMe(atoms,py);freeMe(atoms,pz);
   freeMe(atoms,fx);freeMe(atoms,fy);freeMe(atoms,fz);
   freeMe(atoms,U);
   comdFree(atoms);
}
...
void setVcm(SimFlat* s, real_t newVcm[3])
{
   ...
   for (int iBox=0; iBox<s->boxes->nLocalBoxes; ++iBox)
   {
      for (int iOff=MAXATOMS*iBox, ii=0; ii<s->boxes->nAtoms[iBox]; ++ii, ++iOff)
      {
         ...
         s->atoms->px[iOff] += mass * vShift[0];
         s->atoms->py[iOff] += mass * vShift[1];
         s->atoms->pz[iOff] += mass * vShift[2];
      }
   }
}
...
void setTemperature(SimFlat* s, real_t temperature)
{
   ...
   for (int iBox=0; iBox<s->boxes->nLocalBoxes; ++iBox)
   {
      for (int iOff=MAXATOMS*iBox, ii=0; ii<s->boxes->nAtoms[iBox]; ++ii, ++iOff)
      {
         ...
         s->atoms->px[iOff] = mass * sigma * gasdev(&seed);
         s->atoms->py[iOff] = mass * sigma * gasdev(&seed);
         s->atoms->pz[iOff] = mass * sigma * gasdev(&seed);
      }
   }
   ...
   for (int iBox=0; iBox<s->boxes->nLocalBoxes; ++iBox)
   {
      for (int iOff=MAXATOMS*iBox, ii=0; ii<s->boxes->nAtoms[iBox]; ++ii, ++iOff)
      {
         s->atoms->px[iOff] *= scaleFactor;
         s->atoms->py[iOff] *= scaleFactor;
         s->atoms->pz[iOff] *= scaleFactor;
      }
   }
   kineticEnergy(s, 0);
...
void randomDisplacements(SimFlat* s, real_t delta)
{
   ...
   for (int iBox=0; iBox<s->boxes->nLocalBoxes; ++iBox)
   {
      for (int iOff=MAXATOMS*iBox, ii=0; ii<s->boxes->nAtoms[iBox]; ++ii, ++iOff)
      {
         ...
         s->atoms->rx[iOff] += (2.0*lcg61(&seed)-1.0) * delta;
         s->atoms->ry[iOff] += (2.0*lcg61(&seed)-1.0) * delta;
         s->atoms->rz[iOff] += (2.0*lcg61(&seed)-1.0) * delta;
      }
   }
}
